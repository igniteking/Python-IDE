...
<?php
    $next_id = "";
    $next_sql = "SELECT * FROM courses WHERE id > $Course_id AND course_category = 'c++' ORDER BY id DESC";
    ...
    if ($next_id == "") {
      ...
      $final_next_id = "c++_ide.php?id=" . $next_id;
    }
    ?>
        <?php
    $pre_id = "";
    $pre_sql = "SELECT * FROM courses WHERE id < $Course_id AND course_category = 'c++' ORDER BY id ASC";
    $next_query = mysqli_query($conn, $next_sql);
    while ($row = mysqli_fetch_assoc($next_query)) {
      $pre_id = $row['id'];
    }
    if ($pre_id == "") {
      $final_pre_id = "c++_category.php";
    } else {
      $final_pre_id = "c++_ide.php?id=" . $pre_id;
    }
?>
<?php
...
<div class="row mt-12">
    <div class="col-md-4"><a href='<?php echo $final_pre_id?>' style='text-decoration: underline; font-size:large;'> << Back </a></div>
      ...
      if($result_check == 0) {
        ...
        <form action='c++_ide.php?id=$id' method='POST'>
