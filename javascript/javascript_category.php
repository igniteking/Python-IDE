...
<head>
    ...
    <title>Javascript Modules Index - GlowEdu</title>
    <?php
...
<div class='row mt-12'>
<h2 class='col-6'>Javascript Modules Index</h2>
<?php
         ...
         if($row_count > 0) {
          $normal_row_count = "SELECT * FROM `request` WHERE name = '$user' AND course_name = 'javascript'";
          $normal_query = mysqli_query($conn, $normal_row_count);
          $normale_row_count = mysqli_num_rows($normal_query);
          if ($normale_row_count == '0') {
            echo "<form action='javascript_category.php' class='col-6' method='post'>
      ...
      $query_delete_subscription = "INSERT INTO `request`(`id`, `name`, `email`, `course_name`) VALUES (NULL ,'$user','$email','javascript')";
...
<?php
        ...
        <?php  if ($ut == 'student') {
          if($active == 1) {
              ...
              <a href='javascript_module.php?id=$id'><div class='card-1' style=' overflow-y: scroll; padding: 20px;'><b class='col-md-8' style='font-size: 20px;'>$cat_name</b></div></a>
              </div>";
        } else {
          echo "<div id='card' class='col-md-4' style='margin-top: 15px; '>
          <a href='javascript_module.php?id=$id'><div class='card-1' style=' overflow-y: scroll; padding: 20px;'><b class='col-md-8' style='font-size: 20px;'>$cat_name</b></div></a>
