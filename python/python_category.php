...
<div class='row mt-12'>
<h2 class='col-6'>Python Modules Index</h2>
<?php
         ...
         if($row_count > 0) {
          $normal_row_count = "SELECT * FROM `request` WHERE name = '$user' AND course_name = 'python'";
          $normal_query = mysqli_query($conn, $normal_row_count);
          $normale_row_count = mysqli_num_rows($normal_query);
          if ($normale_row_count == '0') {
            echo "<form action='python_category.php' class='col-6' method='post'>
      ...
      $query_delete_subscription = "INSERT INTO `request`(`id`, `name`, `email`, `course_name`) VALUES (NULL ,'$user','$email','python')";
